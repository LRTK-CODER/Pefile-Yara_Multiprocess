rule System_Tools{
    meta:
        info = "Monitoring Tools Strings"

    strings:
        $wireshark = "wireshark.exe" nocase wide ascii
        $ethereal = "ethereal.exe" nocase wide ascii
        $netstat = "netstat.exe" nocase wide ascii
        $taskm = /taskm(an|gr|on).exe/ nocase wide ascii
        $regedit = /regedit(32)?.exe/ nocase wide ascii
        $sc = "sc.exe" nocase wide ascii
        $procexp = "procexp.exe" nocase wide ascii
        $procmon = "procmon.exe" nocase wide ascii
        $netmon = "netmon.exe" nocase wide ascii
        $regmon = "regmon.exe" nocase wide ascii
        $filemon = "filemon.exe" nocase wide ascii
        $msconfig = "msconfig.exe" nocase wide ascii
        $vssadmin = "vssadmin.exe" nocase wide ascii
        $bcdedit = "bcdedit.exe" nocase wide ascii
        $dumpcap = "dumpcap.exe" nocase wide ascii
        $tcpdump = "tcpdump.exe" nocase wide ascii
		$mshta = "mshta.exe" nocase wide ascii    // Used by DUBNIUM to download files
        $control = "control.exe" nocase wide ascii  // Used by EquationGroup to launch DLLs
        $regsvr32 = "regsvr32.exe" nocase wide ascii
        $rundll32 = "rundll32.exe" nocase wide ascii
...
rule Reversing_Tools{
    meta:
        info = "Reversing Tools Name Strings"

    strings:
        $ida = /ida(q)?(64)?.exe/ nocase wide ascii
        $ImmunityDebugger = "ImmunityDebugger.exe" nocase wide ascii
        $ollydbg = "ollydbg.exe" nocase wide ascii
        $lordpe = "lordpe.exe" nocase wide ascii
        $peid = "peid.exe" nocase wide ascii
...
rule Sandboxie_Detection : AntiVM{
    meta:
        description = "Sandboxie 탐지 Strings"
...
rule Obfuscated_Strings{
	meta:
		info = "obfuscated function name strings"
...
rule BITS_CLSID{
    meta:
        info = "백그라운드 인텔리전트 전송 서비스 strings"
